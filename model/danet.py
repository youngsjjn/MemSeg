...
class DANet(nn.Sequential):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True,
                 backbone='drn', output_stride=8, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True):
    ...
    def forward(self, x, y=None):
        ...
        if self.training:
            ...
            aux_loss = self.criterion(aux_x, y)
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(0):
        model = DANet(layers=18, classes=19,
                      output_stride=16, pretrained=False).cuda()
