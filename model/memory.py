...
class Memory(nn.Module):
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            ...
            return updated_query.contiguous(), updated_memory, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss
        ...
        # test
            ...
            return updated_query.contiguous()
    ...
    def read(self, query, updated_memory):
        ...
        # concat_memory = self.lin(updated_memory.transpose(1,0)).transpose(1,0)  # (b X h X w) X d
        updated_query = query_reshape + self.gamma*concat_memory  # (b X h X w) X 2d
