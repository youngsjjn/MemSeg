...
class TransformNet_mem(nn.Module):
    def __init__(self, layers=50, bins=(1, 3, 6, 8), dropout=0.2, classes=2, zoom_factor=8,
        ...
        super(TransformNet_mem, self).__init__()
    ...
    def forward(self, x, y=None, keys=F.normalize(torch.rand((24, 256), dtype=torch.float), dim=1).cuda()):
        x_size = x.size()
        ...
        if self.training:
            ...
            return x.max(1)[1], main_loss, aux_loss + 0.3 * tr_aux1_loss, separateness_loss, compactness_loss, keys
        else:
            return x
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(0):
        ...
        model = TransformNet(layers=101, bins=(1, 3, 6, 8), dropout=0.1, classes=19, zoom_factor=1, use_aspp=True,
