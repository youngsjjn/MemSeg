...
def main():
    ...
    if not args.has_prediction:
        if args.arch == 'psp':
            ...
            model = PSPNet_mem(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                              backbone=args.backbone, output_stride=args.output_stride, memory_size=args.mem_size, pretrained=False)
        elif args.arch == 'deeplabv3mem':
            from model.deeplabv3_mem import Deeplabv3_mem
            model = Deeplabv3_mem(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                              backbone=args.backbone, output_stride=args.output_stride, memory_size=args.mem_size, pretrained=False)
        elif args.arch == 'danetmem':
            from model.danet_mem import DANet_mem
            model = DANet_mem(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                              backbone=args.backbone, output_stride=args.output_stride, memory_size=args.mem_size, pretrained=False)
        elif args.arch == 'hrnetmem':
            from model.hrnet_mem import HighResolutionNet_mem
            model = HighResolutionNet_mem(args, memory_size=args.mem_size)
            ...
            model = TransformNet_mem(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                              backbone=args.backbone, output_stride=args.output_stride, memory_size=args.mem_size)
        logger.info(model)
        ...
        global m_items
        m_items = torch.load(args.mitem_path, map_location=lambda storage, loc: storage.cuda())
        logger.info("=> loading Memory '{}'".format(args.mitem_path))
...
def net_process(model, image, mean, std=None, flip=True):
    ...
    with torch.no_grad():
        output = model(input,keys=m_items)
