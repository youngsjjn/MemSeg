...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/rellis/rellis_deeplab18.yaml', help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'psp':
        ...
        model = PSPNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor,
                       backbone=args.backbone, output_stride=args.output_stride, criterion=criterion)
        ...
        model = Deeplabv3(layers=args.layers, classes=args.classes,
                            zoom_factor=args.zoom_factor, backbone=args.backbone,
                            output_stride=args.output_stride, criterion=criterion)
        ...
        model = DANet(layers=args.layers, classes=args.classes,
                            zoom_factor=args.zoom_factor, backbone=args.backbone,
                            output_stride=args.output_stride, criterion=criterion)
        if args.backbone == 'mobilenet':
            modules_ori = [model.mobilenet]
        ...
        model = HighResolutionNet(args, criterion=criterion)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            if not ((epoch_log - 2) % 20 == 0 and args.epochs * 0.28 < epoch_log < args.epochs * 0.77):
